 ...
 *
 * You can then set a number of processing publiciables to act on the file.
 ...
 *
 * Then you call the public function {@link process} to actually perform the actions
 ...
 * The processing publiciables will be reset once it is done.
 *
 * You can repeat setting up a new set of processing publiciables,
 ...
 * </ul>
 *  The following publiciables are used only if {@link image_resize} == true
 * <ul>
 ...
 *   - make sure exec() and ini_get_all()public function are not disabled if we want to use them<br>
 ...
 *   - replaced ereg public functions to be compatible with PHP 5.3<br>
 ...
 *   - the class can now output the uploaded files content as the return value of process() if the public function is called with an empty or null argumenti, or no argument</li>
 ...
 *   - removed inline public functions to be fully PHP5 compatible </li>
...
class Upload
{
    ...
    /**
     * Set this publiciable to false in the init() public function if you don't want to check the MIME
     ...
     *
     * The public function mime_content_type() will be deprecated,
    ...
    /**
     * Init or re-init all the processing publiciables to their default values
     *
     * This public function is called in the constructor, and after each call of {@link process}
     *
     * @access private
     */
    public function init()
    {
        // overiddable publiciables
     ...
     *
     * If the file has been uploaded, the constructor will populate all the publiciables holding the upload
     * information (none of the processing class publiciables are used here).
    ...
    public function __construct($file, $lang = 'en_GB')
    {
        ...
        if ($this->uploaded) {
                        ...
                        if ($this->mime_fileinfo !== '') {
                            if ($this->mime_fileinfo === true) {
                                if (getenv('MAGIC') === FALSE) {
                                    ...
                                    $path = getenv('MAGIC');
                                    $this->log .= '&nbsp;&nbsp;&nbsp;&nbsp;MAGIC path is set to ' . $path . ' from MAGIC publiciable<br />';
                                }
                            } else {
                ...
                if ($this->mime_file) {
                    ...
                    if (substr(PHP_OS, 0, 3) != 'WIN') {
                        if (function_exists('exec')) {
                            ...
                            $this->log .= '&nbsp;&nbsp;&nbsp;&nbsp;PHP exec() public function is disabled<br />';
                        }
                    } else {
            ...
            if (!$this->file_src_mime || !is_string($this->file_src_mime) || empty($this->file_src_mime) || strpos($this->file_src_mime, '/') === FALSE) {
                if ($this->mime_getimagesize) {
                    ...
                    if (is_array($info) && array_key_exists('mime', $info)) {
                        ...
                        $this->log .= '&nbsp;&nbsp;&nbsp;&nbsp;MIME type detected as ' . $this->file_src_mime . ' by PHP getimagesize() public function<br />';
                        if (@preg_match("/^([\.-\w]+)\/([\.-\w]+)(.*)$/i", $this->file_src_mime)) {
                            $this->file_src_mime = preg_replace("/^([\.-\w]+)\/([\.-\w]+)(.*)$/i", '$1/$2', $this->file_src_mime);
                            $this->log .= '-&nbsp;MIME validated as ' . $this->file_src_mime . '<br />';
                        } else {
            ...
            $this->log .= '<b>source publiciables</b><br />';
    ...
    /**
     * Actually uploads the file, and act on it according to the set processing class publiciables
     *
     * This public function copies the uploaded file to the given location, eventually performing actions on it.
     ...
     *
     * According to the processing class publiciables set in the calling file, the file can be renamed,
     ...
     * processing class publiciables will be reset to their default value.
     * This allows you to set new properties, and perform another {@link process} on the same uploaded file
     *
     * If the public function is called with a null or empty argument, then it will return the content of the picture
     *
    ...
    public function process($server_path = null)
    {
        ...
        // copy some publiciables as we need to keep them clean
        ...
        if ($this->processed) {
            ...
            // repopulate dst publiciables from src
            ...
            $this->log .= '- destination publiciables<br />';
            ...
            if ($image_manipulation) {
                ...
                if ($this->processed && $image_src) {
                    ...
                    if ($this->image_resize) {
                        ...
                        } elseif (is_numeric($this->image_ratio_pixels)) {
                        ...
                        } elseif ($this->image_ratio || $this->image_ratio_crop || $this->image_ratio_fill || $this->image_ratio_no_zoom_in || $this->image_ratio_no_zoom_out) {
                            ...
                            if ((!$this->image_ratio_no_zoom_in && !$this->image_ratio_no_zoom_out) || ($this->image_ratio_no_zoom_in && ($this->image_src_x > $this->image_x || $this->image_src_y > $this->image_y)) || ($this->image_ratio_no_zoom_out && $this->image_src_x < $this->image_x && $this->image_src_y < $this->image_y)) {
                                ...
                                if ($this->image_ratio_crop) {
                                    ...
                                    if (($this->image_src_x / $this->image_x) > ($this->image_src_y / $this->image_y)) {
                                        ...
                                        } elseif (strpos($this->image_ratio_crop, 'r') !== false) {
                                        ...
                                        } elseif (strpos($this->image_ratio_crop, 'b') !== false) {
                                    ...
                                    if (($this->image_src_x / $this->image_x) < ($this->image_src_y / $this->image_y)) {
                                        ...
                                        if (strpos($this->image_ratio_fill, 'l') !== false) {
                                            $ratio_crop['l'] = 0;
                                            $ratio_crop['r'] = $ratio_crop['x'];
                                        } elseif (strpos($this->image_ratio_fill, 'r') !== false) {
                                            $ratio_crop['l'] = $ratio_crop['x'];
                                            $ratio_crop['r'] = 0;
                                        } else {
                                            $ratio_crop['l'] = round($ratio_crop['x'] / 2);
                                        ...
                                        if (strpos($this->image_ratio_fill, 't') !== false) {
                                            $ratio_crop['t'] = 0;
                                            $ratio_crop['b'] = $ratio_crop['y'];
                                        } elseif (strpos($this->image_ratio_fill, 'b') !== false) {
                                            $ratio_crop['t'] = $ratio_crop['y'];
                                            $ratio_crop['b'] = 0;
                                        } else {
                                            $ratio_crop['t'] = round($ratio_crop['y'] / 2);
                    ...
                    if ($gd_version >= 2 && is_numeric($this->image_frame)) {
                        ...
                        for ($i = 0; $i < $nb; $i++) {
                            list($red, $green, $blue) = $this->getcolors($publics[$i]);
                    ...
                    if ($this->image_watermark != '' && file_exists($this->image_watermark)) {
                        ...
                        } elseif ($watermark_type == IMAGETYPE_JPEG) {
                            if (!function_exists('imagecreatefromjpeg')) {
                        ...
                        } elseif ($watermark_type == IMAGETYPE_BMP) {
                            if (!method_exists($this, 'imagecreatefrombmp')) {
                        ...
                        if ($watermark_checked) {
                            ...
                            if (is_numeric($this->image_watermark_x)) {
                                ...
                                } elseif (strpos($this->image_watermark_position, 'l') !== false) {
                                    $watermark_x = 0;
                                } else {
                            ...
                            if (is_numeric($this->image_watermark_y)) {
                                ...
                                } elseif (strpos($this->image_watermark_position, 't') !== false) {
                                    $watermark_y = 0;
                                } else {
                    ...
                    if (!empty($this->image_text)) {
                        ...
                        if (is_numeric($this->image_text_x)) {
                            ...
                            } elseif (strpos($this->image_text_position, 'l') !== false) {
                                $text_x = 0;
                            } else {
                        ...
                        if (is_numeric($this->image_text_y)) {
                            ...
                            } elseif (strpos($this->image_text_position, 't') !== false) {
                                $text_y = 0;
                            } else {
     ...
     *
     * You might want not to use this public function if you work on local files, as it will delete the source file
     *
     * @access public
     */
     ...
     * Opens a BMP image
     *
     * This public function has been written by DHKold, and is used with permission of the author
     *
     * @access public
     */
     ...
     * Saves a BMP image
     *
     * This public function has been published on the PHP website, and can be used freely
     *
     * @access public
     */
